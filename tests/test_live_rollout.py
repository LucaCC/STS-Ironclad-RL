...
class PlannedPolicy:
    ...
    def select_action(self, observation: EncodedObservation) -> ActionDecision:
        return self.decisions[observation.features["turn"] - 1]
...
def test_live_episode_runner_completes_episode_and_records_metrics() -> None:
    bridge, transport, session_id = _build_bridge_with_session(
        snapshots=[
            ...
            GameStateSnapshot(
                ...
                floor=2,
                act=1,
                raw_state={"victory": True, "score": 33, "reward": 2.0},
            ),
        ]
    )
    ...
    assert result.metadata["final_floor"] == 2
