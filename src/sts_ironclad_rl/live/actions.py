...
class PlayCardAction:
    ...
    def action_id(self) -> str:
        if self.target is None:
            return f"{LiveActionType.PLAY_CARD.value}:{self.hand_index}"
        return f"{LiveActionType.PLAY_CARD.value}:{self.hand_index}:{self.target.monster_index}"
...
class EndTurnAction:
    ...
    @property
    def action_id(self) -> str:
        return LiveActionType.END_TURN.value
...
class ChooseAction:
    ...
    @property
    def action_id(self) -> str:
        return f"{LiveActionType.CHOOSE.value}:{self.choice_index}"
...
class ProceedAction:
    ...
    @property
    def action_id(self) -> str:
        return LiveActionType.PROCEED.value
...
class LeaveAction:
    ...
    @property
    def action_id(self) -> str:
        return LiveActionType.LEAVE.value
...
class CommunicationModActionContract:
    ...
    def validate_action(self, snapshot: GameStateSnapshot, action: LiveAction) -> None:
        ...
        if isinstance(action, ChooseAction):
            ...
            if action.choice_index >= len(_choice_list(snapshot)):
    ...
    def to_command(self, session_id: str, decision: ActionDecision) -> ActionCommand:
        if decision.arguments:
            ...
            raise ValueError(msg)
        action = action_from_id(decision.action_id)
...
def _choice_list(snapshot: GameStateSnapshot) -> tuple[str, ...]:
