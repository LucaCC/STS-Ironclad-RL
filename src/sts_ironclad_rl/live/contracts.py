"""Live-game RL contracts built on top of the bridge package."""
...
class ActionTraceEntry:
    """Action trace metadata for one live rollout step."""

    step_index: int
    action_id: str
    legal_action_ids: tuple[str, ...]
    command: str
    command_arguments: Mapping[str, Any] = field(default_factory=dict)
    metadata: Mapping[str, Any] = field(default_factory=dict)
...
class RolloutResult:
    ...
    action_trace: tuple[ActionTraceEntry, ...]
    terminal: bool
    step_count: int
...
@dataclass(frozen=True)
class PolicyContext:
    """Execution context supplied to a live policy at each step."""
